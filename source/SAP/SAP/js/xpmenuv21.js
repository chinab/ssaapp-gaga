        ...
        if (String(divs.item(dn).className).substring(0, 7) == "TopItem") {
            ...
            try //to apply cookies settings
			{
			    if (menuStateAry != 0) {
			        ...
			        //itemContainerDiv.style.display = "table";
			    }
                ...
                if (menuStateAry != 0) {
                    if (parseInt(menuStateAry[aryNum]) == 0)
                        ...
                        itemContainerDiv.style.display = 'inline'; //inline
                }
            }
            catch (e) {
...
function Init(objDiv) {
    ...
    for (r = 0; r < document.styleSheets.length; r++) {
        if (-1 != String(document.styleSheets[r].href).indexOf("leftNavigation.css"))
            break;
    }
    ...
    // Extend: enlarge padding bottom of the last menu item
    if (lastmenuNum != -1 && itemContainerDiv.childNodes.length >= lastmenuNum)
        itemContainerDiv.childNodes.item(lastmenuNum).style.paddingBottom = "12px";
...
function RunSlide(objIndex) {
    ...
    if (!menuObjArray[6][objIndex]) {
        ...
        if (menuObjArray[4][objIndex] < (menuObjArray[2][objIndex] * divHeight))
            ...
            if (menuObjArray[0][objIndex].style.removeProperty)
                menuObjArray[0][objIndex].style.removeProperty("height");
            else
                menuObjArray[0][objIndex].style.removeAttribute("height");
            menuObjArray[1][objIndex].style.display = 'inline'; // inline
            clearInterval(menuObjArray[5][objIndex]);
            menuObjArray[5][objIndex] = null;
